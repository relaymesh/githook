...
The `relaymesh` binary runs the server and provides a CLI for Connect RPC endpoints.
...
```bash
relaymesh init --config server.yaml --profile server
...
- `RELAYMESH_API_KEY`: send `x-api-key` if your server expects API-key auth
- `RELAYMESH_AUTH_TOKEN`: override the cached auth token
- `RELAYMESH_TOKEN_CACHE`: override the token cache path
...
```bash
relaymesh --endpoint http://localhost:8080 namespaces list --provider github
relaymesh --endpoint http://localhost:8080 namespaces list --provider github --state-id <state-id>
relaymesh --endpoint http://localhost:8080 namespaces sync --provider gitlab
relaymesh --endpoint http://localhost:8080 namespaces sync --provider gitlab --state-id <state-id>
relaymesh --endpoint http://localhost:8080 namespaces webhook get --provider gitlab --repo-id <repo-id>
relaymesh --endpoint http://localhost:8080 namespaces webhook get --provider gitlab --repo-id <repo-id> --state-id <state-id>
relaymesh --endpoint http://localhost:8080 namespaces webhook set --provider gitlab --repo-id <repo-id> --enabled
relaymesh --endpoint http://localhost:8080 namespaces webhook set --provider gitlab --repo-id <repo-id> --enabled --state-id <state-id>
```
...
relaymesh --endpoint http://localhost:8080 rules get --id <rule-id>
relaymesh --endpoint http://localhost:8080 rules create --when 'action == "opened"' --emit pr.opened.ready --driver-id <driver-id>
relaymesh --endpoint http://localhost:8080 rules update --id <rule-id> --when 'action == "closed"' --emit pr.merged --driver-id <driver-id>
relaymesh --endpoint http://localhost:8080 rules delete --id <rule-id>
```
...
`--driver-id` should match the driver record ID (see `relaymesh drivers list`).

## Providers

```bash
relaymesh --endpoint http://localhost:8080 providers list
relaymesh --endpoint http://localhost:8080 providers get --provider github --hash <instance-hash>
relaymesh --endpoint http://localhost:8080 --tenant-id acme providers create --provider github --config-file github.yaml
...
relaymesh --endpoint http://localhost:8080 --tenant-id acme drivers create --name amqp --config-file amqp.yaml
...
Use `relaymesh drivers list` to see driver record IDs for `rules create --driver-id`.
